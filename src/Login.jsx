...
/**
 * ASSETFLOW REDESIGN - REACT ARCHITECTURE (Full Auth & Dashboard)
 * 
...
const LoginForm = ({ onToggleAuth, onAuthSuccess }) => (
  <div className="w-full max-w-md bg-white border border-[#E2E8F0] rounded-2xl p-10 shadow-xl shadow-blue-900/5 z-10">
    ...
    <p className="text-[#64748B] text-center mb-8 text-sm font-medium">Sign in to manage your corporate assets</p>
      ...
      <div className="space-y-2">
        ...
        <div className="relative group">
          <input 
            ...
            className="w-full bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg px-4 py-3 text-[#1E293B] text-sm focus:outline-none focus:bg-white focus:ring-2 focus:ring-[#0077B5]/20 focus:border-[#0077B5] transition-all" 
          />
          <Mail className="absolute right-3 top-1/2 -translate-y-1/2 text-[#94A3B8] group-focus-within:text-[#0077B5]" size={18} />
        </div>
      </div>
      
      <div className="space-y-2">
        <div className="flex justify-between items-center px-1">
          <label className="text-[10px] font-bold text-[#64748B] uppercase tracking-widest">Password</label>
          <button type="button" className="text-[#0077B5] text-[10px] font-bold uppercase tracking-wider hover:underline">Forgot password?</button>
        </div>
        <div className="relative group">
          <input 
            type="password" 
            ...
            className="w-full bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg px-4 py-3 text-[#1E293B] text-sm focus:outline-none focus:bg-white focus:ring-2 focus:ring-[#0077B5]/20 focus:border-[#0077B5] transition-all" 
          />
          <Lock className="absolute right-3 top-1/2 -translate-y-1/2 text-[#94A3B8] group-focus-within:text-[#0077B5]" size={18} />
        </div>
      </div>

      <button className="w-full bg-[#0077B5] text-white font-bold py-3.5 rounded-lg hover:bg-[#005F91] transition-all uppercase tracking-widest text-sm shadow-lg shadow-blue-500/20 active:scale-[0.98]">
        Sign In
      </button>
    </form>
    ...
    <div className="mt-8 pt-8 border-t border-[#E2E8F0] text-center">
      <div className="flex items-center gap-4 mb-6">
        ...
        <span className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest">New here?</span>
        <div className="h-px flex-1 bg-[#E2E8F0]"></div>
      </div>
      
      <div className="bg-[#F8FAFC] rounded-xl p-4 mb-6">
        <p className="text-[11px] text-[#64748B] leading-relaxed">
          Sign up creates an employee account. Admin roles are assigned later by your organization.
        </p>
      </div>

      <button 
        onClick={onToggleAuth} 
        className="w-full border-2 border-[#E2E8F0] text-[#1E293B] font-bold py-3 rounded-lg hover:bg-[#F8FAFC] transition-colors uppercase tracking-widest text-xs"
      >
        Create Account
      </button>
    </div>
...
const SignUpForm = ({ onToggleAuth, onAuthSuccess }) => (
  <div className="w-full max-w-md bg-white border border-[#E2E8F0] rounded-2xl p-10 shadow-xl shadow-blue-900/5 z-10">
    ...
    <p className="text-[#64748B] text-center mb-8 text-sm font-medium">Start managing your global assets with precision.</p>
    ...
    <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); onAuthSuccess(); }}>
      <div className="space-y-2">
        ...
        <div className="relative group">
          ...
          <input 
            ...
            className="w-full bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg pl-10 pr-4 py-3 text-[#1E293B] text-sm focus:outline-none focus:bg-white focus:ring-2 focus:ring-[#0077B5]/20 focus:border-[#0077B5] transition-all" 
          />
        </div>
      </div>
      <div className="space-y-2">
        <label className="text-[10px] font-bold text-[#64748B] uppercase tracking-widest ml-1">Work Email</label>
        <div className="relative group">
      ...
      <div className="space-y-2">
        <label className="text-[10px] font-bold text-[#64748B] uppercase tracking-widest ml-1">Create Password</label>
        <div className="relative group">
          <Lock className="absolute left-3 top-1/2 -translate-y-1/2 text-[#94A3B8] group-focus-within:text-[#0077B5]" size={18} />
          <input 
            type="password" 
            placeholder="••••••••"
            className="w-full bg-[#F8FAFC] border border-[#E2E8F0] rounded-lg pl-10 pr-10 py-3 text-[#1E293B] text-sm focus:outline-none focus:bg-white focus:ring-2 focus:ring-[#0077B5]/20 focus:border-[#0077B5] transition-all" 
          />
          <button type="button" className="absolute right-3 top-1/2 -translate-y-1/2 text-[#94A3B8] hover:text-[#64748B]">
            <Eye size={18} />
          </button>
        </div>
        <p className="text-[10px] text-[#64748B] flex items-center gap-1.5 mt-1 font-medium ml-1">
          <HelpCircle size={12} />
          Minimum 12 characters, including numbers and symbols
        </p>
      </div>
      <button className="w-full bg-[#0077B5] text-white font-bold py-3.5 rounded-lg hover:bg-[#005F91] transition-all flex items-center justify-center gap-2 uppercase tracking-widest text-sm shadow-lg shadow-blue-500/20 active:scale-[0.98]">
  ...
  if (authState !== 'authenticated') {
    return (
      <div className="min-h-screen bg-[#F7FAFC] flex items-center justify-center p-6 relative overflow-hidden font-inter">
        ...
        <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-blue-50/60 rounded-full blur-[100px] translate-y-1/2 -translate-x-1/2" />
        
        {/* Subtle dot grid pattern */}
        <div className="absolute inset-0 opacity-[0.03] pointer-events-none" 
             style={{ backgroundImage: 'radial-gradient(#0077B5 1px, transparent 0)', backgroundSize: '24px 24px' }} 
        />

        {authState === 'login' ? (
          <LoginForm 
            onToggleAuth={() => setAuthState('signup')} 
        ...
        <footer className="absolute bottom-8 w-full text-center">
           <p className="text-[#94A3B8] text-[10px] uppercase tracking-[0.2em] font-bold">
             © 2024 AssetFlow Technologies Inc. All rights reserved.
           </p>
        </footer>
      </div>
    );
      ...
      <div className="ml-64 flex flex-col min-h-screen">
        ...
        <main className="flex-1 overflow-y-auto p-8 space-y-8">
          ...
          <div className="bg-[#FEF2F2] border border-[#FEE2E2] rounded-xl p-4 flex items-center justify-between shadow-sm">
            ...
            <button className="bg-[#B91C1C] text-white text-xs font-bold px-4 py-2 rounded-lg hover:bg-[#991B1B] transition-colors uppercase tracking-wider">
              Review Now
            </button>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className="bg-white border border-[#E2E8F0] rounded-2xl p-6 relative overflow-hidden shadow-sm group hover:shadow-md transition-all">
          ...
          <div className="h-64 flex flex-col items-center justify-center border-2 border-dashed border-[#E2E8F0] rounded-2xl bg-white/50 text-[#94A3B8] font-semibold">
            <Building2 size={40} className="mb-4 opacity-20" />
            <p className="italic">Detailed activity modules are being migrated to the redesign...</p>
          </div>
        </main>
